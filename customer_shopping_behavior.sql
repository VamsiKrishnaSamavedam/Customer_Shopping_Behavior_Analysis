...
-- for type casting the columns in sql use "review_rating::numeric"
...
select subscription_status,count(customer_id),sum(purchase_amount) as "total_revenue",round(avg(purchase_amount),2) as "average_spend"
from customer
...
--Q7. Segment customers into 'New','Loyal','Returning' based on their total number of previous purchases and show the count.
-- in this case when we need to segment or make groups we need to use CTE's
...
select customer_segments, count(customer_segments)
from customer_type
